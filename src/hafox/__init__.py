...
def fetch_smartfox_data(url: str = 'http://smartfox/values.xml', timeout: int = 5) -> Optional[str]:
...
def create_overview_table(values: Dict[str, str]) -> Table:
    ...
    table = Table(title="SmartFox Energy Monitor", show_header=True, header_style="bold magenta")
    table.add_column("Category", style="cyan", width=20)
    table.add_column("Metric", style="green")
    table.add_column("Value", style="yellow", justify="right")
    
    # System Info
    table.add_row("System", "Date/Time", f"{values.get('dateValue', 'N/A')} {values.get('timeValue', 'N/A')}")
    ...
    if 'batterySoc' in values and values['batterySoc'] != '-1%':
        ...
        table.add_row("Battery", "Temperature", values.get('battery1Temperature', 'N/A'))
...
def display_simple(values: Dict[str, str]) -> None:
    ...
    sections = {
        'SYSTEM INFO': ['dateValue:Date', 'timeValue:Time', 'ipAddress:IP Address', 'version:Firmware'],
        'GRID STATUS': ['hidPower:Grid Power', 'toGridValue:To Grid', 'detailsPowerValue:Current Power'],
        'ENERGY': ['energyValue:Total Energy', 'eDayValue:Energy Today', 'eToGridValue:Energy To Grid Total', 'eDayToGridValue:Energy To Grid Today'],
        'SOLAR PRODUCTION': ['hidProduction:Production', 'wr1PowerValue:Inverter 1 Power', 'wr1EnergyValue:Inverter 1 Energy']
    }
...
@click.option('--format', 'output_format', type=click.Choice(['rich', 'simple', 'json']), default='rich', help='Output format')
...
def monitor(url: str, output_format: str, watch: bool, interval: int):
    ...
    if watch:
        console.print(f"[green]Watching SmartFox data (refresh every {interval}s, press Ctrl+C to stop)...[/green]")
        try:
            while True:
                console.clear()
                display_once()
